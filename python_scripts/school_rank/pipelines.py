...
class CsvPipeline:
    def process_item(self, item, spider):
        ...
        df_dict(d_item['table_aux']['retention_rate']['national_average']
                ).to_csv('./data/output/ret_rate_national.csv')
        df_dict(d_item['table_aux']['indicators']['school_average']
                ).to_csv('./data/output/indicator_school.csv')
        df_dict(d_item['table_aux']['indicators']['national_average']
                ).to_csv('./data/output/indicator_national.csv')
        df_dict(d_item['table_aux']['parents_education']
                ).to_csv('./data/output/parental_edu.csv')
        df_dict(d_item['table_aux']['ranking']
                ).to_csv('./data/output/ranking.csv')
...
class SqlitePipeline:
    def process_item(self, item, spider):
        ...
        df_dict(d_item['table_aux']['retention_rate']['national_average']
                ).to_sql('ret_rate_national', con=engine)
        df_dict(d_item['table_aux']['indicators']['school_average']
                ).to_sql('indicator_school', con=engine)
        df_dict(d_item['table_aux']['indicators']['national_average']
                ).to_sql('indicator_national', con=engine)
        df_dict(d_item['table_aux']['parents_education']
                ).to_sql('parental_edu', con=engine)
        df_dict(d_item['table_aux']['ranking']
                ).to_sql('ranking', con=engine)
